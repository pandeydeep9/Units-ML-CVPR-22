...
def rotate_query_image(image,angle):
    """
    Rotate the input images by specified angle. Currenlty, works for omniglot
    ...
    for index, img in enumerate(image):
        #print(img.shape)
        qimage = transforms.RandomRotation(angle,fill=(255,))(F.to_pil_image(img[0]))
        qimage= qimage.resize((28, 28), resample=Image.LANCZOS)
        image[index] = torch.FloatTensor(np.asarray(qimage))
    return image

def scale_query_image(image,scaling_factor):
    """
    Scale the input images by specified scaling factor. Currenlty, works for omniglot
    :param image: the input image
    :param scaling_factor: the factor (multiplier) to scale the image by
    :return: rotated images
    """

    for index, img in enumerate(image):
        #print(img.shape)
        qimage = transforms.RandomAffine(degrees=0, scale=scaling_factor, fillcolor=(255,))(F.to_pil_image(img[0]))
        qimage= qimage.resize((28, 28), resample=Image.LANCZOS)
        image[index] = torch.FloatTensor(np.asarray(qimage))
    return image

# From source
def augment_image(image, k, channels, augment_bool, args, dataset_name, set_name=""):
...
def get_transforms_for_dataset(dataset_name, args, k):
    if "cifar10" in dataset_name or "cifar100" in dataset_name:
        ...
        #augment training set classes for omniglot
    ...
    elif 'imagenet' in dataset_name or 'mini' in dataset_name:
...
class FewShotLearningDatasetParallel(Dataset):
    ...
    def load_datapaths(self):
        ...
        """
        dataset_dir = "datasets/"  # os.environ['DATASET_DIR']
    ...
    def load_image(self, image_path, channels):
        ...
        if not self.data_loaded_in_memory:
            ...
            if 'omniglot' in self.dataset_name or 'mnist' in self.dataset_name:
    ...
    def get_set(self, dataset_name, seed, augment_images=False):
        ...
        rng = np.random.RandomState(seed)
            # Uniform sampling of classes to make the task distribution
        ...
        k_list = rng.randint(0, 4, size=self.num_classes_per_set) #always confused what this does??
        #k list is a list of len num_classes_per_set with values from 0 to 4
        #and is used for augmentation (0,1,2,3) of classes for omniglot dataset :)
        ...
        for class_entry in selected_classes:
            if self.current_set_name == "train":
                ...
                choose_samples_list = rng.choice(self.dataset_size_dict[dataset_name][class_entry],
                                             size=self.num_samples_per_class + \
                                                  self.num_target_samples * self.args.select_from_tasks\
                                                 , replace=False)
            else:
                # print("not tr")
                choose_samples_list = rng.choice(self.dataset_size_dict[dataset_name][class_entry],
            ...
            for sample in choose_samples_list:
                ...
                x_class_data = augment_image(image=x_class_data, k=k,
                                             ...
                                             set_name=self.current_set_name)
        ...
        return support_set_images, target_set_images, support_set_labels, target_set_labels, seed, selected_classes
    ...
    def __getitem__(self, idx):
        ...
        support_set_images, target_set_image, support_set_labels, target_set_label, seed, classes_sel = \
            self.get_set(self.current_set_name, seed=self.seed[self.current_set_name] + idx,
                         augment_images=self.augment_images)

        # Return selected classes as well
        return support_set_images, target_set_image, support_set_labels, target_set_label, seed, classes_sel
